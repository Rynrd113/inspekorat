...
@section('header', 'Edit Foto Slider')
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="bg-white rounded-lg shadow-md border border-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('admin.hero-sliders.update', $heroSlider) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <div class="relative inline-block">
                        <img src="{{ $heroSlider->gambar_url }}" alt="{{ $heroSlider->judul }}" 
                ...
                <div class="mb-6">
                    <label for="gambar" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $heroSlider->gambar ? 'Ganti Foto' : 'Foto Slider' }} 
                        @if(!$heroSlider->gambar)<span class="text-red-500">*</span>@endif
                    </label>
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-blue-400 transition-colors" id="dropzone">
                        <div class="space-y-2 text-center">
                            <div id="preview-container" class="hidden mb-4">
                                <img id="preview-image" src="" alt="Preview" class="mx-auto max-h-48 rounded-lg shadow-md">
                            </div>
                            <div id="upload-icon">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="judul" name="judul" value="{{ old('judul', $heroSlider->judul) }}"
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="subjudul" name="subjudul" value="{{ old('subjudul', $heroSlider->subjudul) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div>
                        ...
                        <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $heroSlider->urutan) }}" min="0"
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500">
                            <option value="published" {{ old('status', $heroSlider->status) == 'published' ? 'selected' : '' }}>Published</option>
                            <option value="draft" {{ old('status', $heroSlider->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                        </select>
                    </div>

                    <div class="flex items-end pb-2">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $heroSlider->is_active) ? 'checked' : '' }}
                ...
                <div class="flex justify-between pt-6 border-t border-gray-200">
                    <button type="button" onclick="confirmDelete()" class="px-4 py-2 bg-red-100 text-red-700 rounded-md hover:bg-red-200 transition-colors">
            ...
            <!-- Delete Form (Hidden) -->
            <form id="delete-form" action="{{ route('admin.hero-sliders.destroy', $heroSlider) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
...
function confirmDelete() {
    if (confirm('Yakin ingin menghapus foto slider ini?')) {
